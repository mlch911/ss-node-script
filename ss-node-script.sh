...
ServerSetup_Shell(){
	...
	sed -i -n "2c NODE_ID = ${node_id}" userapiconfig.py
	
	#设置API
	sed -i -n '15c API_INTERFACE = 'glzjinmod'  # glzjinmod, modwebapi' userapiconfig.py
	...
	sed -i -n "24c MYSQL_HOST = '${mysql_host}'" userapiconfig.py
	...
	sed -i -n "24c MYSQL_PORT = ${mysql_port}" userapiconfig.py
	...
	sed -i -n "24c MYSQL_USER = '${mysql_user}'" userapiconfig.py
	...
	sed -i -n "24c MYSQL_PASS = '${mysql_pass}'" userapiconfig.py
	...
	sed -i -n "24c MYSQL_DB = '${mysql_db}'" userapiconfig.py
